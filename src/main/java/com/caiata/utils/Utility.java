...
public class Utility {
    ...
    /**
     * Metodo per scattare lo screen e poi castarlo in Base64 per aggiungerlo al report
     * @return
     */
    public static String getScreenMobile(){
        String sDate = null;
        String base64 = null;
        try{
            SimpleDateFormat oSDF = new SimpleDateFormat("yyyyMMddHHmm");
            sDate = oSDF.format(new Date());
            byte[] imageByte = ((TakesScreenshot) com.caiata.utils.ManagementDriver.getAndroidDriver()).getScreenshotAs(OutputType.BYTES);
